...
//     modelState.pose.position.z = 2.5;
...
//     // path.poses[1].pose.position.x = rand_x_tar;
//     // path.poses[1].pose.position.y = rand_y_tar;
//     path.poses[1].pose.position.x = world_x_tar + x_init;
//     path.poses[1].pose.position.y = world_y_tar + y_init;
//     path.poses[1].pose.orientation.x = q.x();
//     path.poses[1].pose.orientation.y = q.y();
//     path.poses[1].pose.orientation.z = q.z();
//     path.poses[1].pose.orientation.w = q.w();
...
// ros::Publisher pub_stateflag;
// ros::Publisher pub_firstz;
// ros::Publisher pub_initpos;
// ros::Publisher pub_targetflag;
...
// void PubTargetFlag(int _targetflag){
//     std_msgs::Int8 targetflag;
//     targetflag.data = _targetflag;
//     pub_targetflag.publish(targetflag);
// }
...
//         std_msgs::Float32MultiArray initpos;
//         initpos.data.push_back(x_init);
//         initpos.data.push_back(y_init);
//         pub_initpos.publish(initpos);
...
//         global_x_tar = cos(-yaw_init)*world_x_tar - sin(-yaw_init)*world_y_tar; // x target from global frame
//         global_y_tar = sin(-yaw_init)*world_x_tar + cos(-yaw_init)*world_y_tar; // y target from global frame
...
//         // yaw_target = atan2(global_y_tar, global_x_tar);
//         // if(yaw_target >= 0) {
//             // yaw_target_dis = 0.07; // arbitrary contant number    
//         //}
//         // else {
//             // yaw_target_dis = -0.07; // arbitrary contant number    
//         //}
...
//         ROS_ERROR("yaw_target_deg: %f", yaw_target_deg);
//         ROS_ERROR("num_div: %f", num_div);
//         ROS_ERROR("yaw_target_dis: %f", yaw_target_dis);
...
//     if( std::isnan(_yaw_target) == 1 )
//     {
//         ROS_ERROR("yaw_target is NAN!!!");
//     }
...
double cur_x_world = 0;
double cur_y_world = 0;
...
double global_x_tar = 0.0;
double global_y_tar = 0.0;
double global_x_cur = 0;
double global_y_cur = 0;
...
void msgCallbackBodyPose(const std_msgs::Float32MultiArray::ConstPtr& msg)
{
    cur_x_world = msg->data[0];
    cur_y_world = msg->data[1];
    cur_z_world = msg->data[2];//  - 0.04; // 0.43만큼 +되어 있음.
    roll = msg->data[3];
    pitch = msg->data[4];
    yaw = msg->data[5];
}
...
void PublishPath(int _cnt_path){
    ...
    global_y_cur = sin(-yaw_init)*(cur_x_world - x_init) + cos(-yaw_init)*(cur_y_world - y_init);
    ...
    q_tar.setRPY(0, 0, yaw_target+yaw_init);
    
    nav_msgs::Path path_vis;

    path_vis.header.stamp = ros::Time::now();
    path_vis.header.frame_id = "world";
    path_vis.poses.resize(2);  // allocate memory for the poses array
    path_vis.poses[0].header.frame_id = "world";
    path_vis.poses[0].header.stamp = ros::Time::now();
    path_vis.poses[0].pose.position.x = x_init;
    path_vis.poses[0].pose.position.y = y_init;
    path_vis.poses[0].pose.orientation.x = q_init.x();
    path_vis.poses[0].pose.orientation.y = q_init.y();
    path_vis.poses[0].pose.orientation.z = q_init.z();
    path_vis.poses[0].pose.orientation.w = q_init.w();

    path_vis.poses[1].header.frame_id = "world";
    path_vis.poses[1].header.stamp = ros::Time::now();
    path_vis.poses[1].pose.position.x = world_x_tar + x_init;
    path_vis.poses[1].pose.position.y = world_y_tar + y_init;
    path_vis.poses[1].pose.orientation.x = q_tar.x();
    path_vis.poses[1].pose.orientation.y = q_tar.y();
    path_vis.poses[1].pose.orientation.z = q_tar.z();
    path_vis.poses[1].pose.orientation.w = q_tar.w();
    pub_path_vis.publish(path_vis);
...
void print_check(int _cnt, string msg) {
    if (_cnt%10 == 1) {
        int cnt_check = _cnt/10;
        std::cout << msg << " " << cnt_check << "s" << std::endl;
        outfile << msg << "\n";
    }
}

double distance(double x1, double y1, double x2, double y2, double x3, double y3) {
    // Calculate the distance between point (x3, y3) and the line passing through points (x1, y1) and (x2, y2)
    double a = y2 - y1;
    double b = x1 - x2;
